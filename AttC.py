...
if attack == "trail":
    torch.save(backdoor, 'backdoor/CEL_trail.pt')
elif attack == "red":
    torch.save(backdoor, 'backdoor/CEL_red.pt')
...
if attack == "trail":
    ...
    print('DONE TRAINING')
    torch.save(netG.state_dict(), './mod/CELgenTrail.pth')
    torch.save(netD.state_dict(), './mod/CELdisTrail.pth')
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('./imTR/ClossTrail.png')
    plt.show()
    ...
    plt.savefig('./imTR/Cimages_Trail.png')
    plt.show()  
elif attack == "red":
    batch_size = 2000
    iteration = 200
    ...
    print('DONE TRAINING')
    torch.save(netG.state_dict(), './mod/CELgenRED.pth')
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('./imRED/ClossRED.png')
    plt.show()
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('./imRED/CTarDisRED.png')
    plt.show()
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('./imRED/CExpDisRED.png')
    plt.show()

    # save the generated images as GIF file
    to_pil_image = transforms.ToPILImage()
    imgs = [np.array(to_pil_image(img)) for img in img_list]
    imageio.mimsave('./imRED/CGGif_RED.gif', imgs)

    to_pil_image1 = transforms.ToPILImage()
    imgs1 = [np.array(to_pil_image1(img)) for img in att_list]
    imageio.mimsave('./imRED/CAttGif_RED.gif', imgs1)
    #
    # Grab a batch of real images from the dataloader
    real_batch = next(iter(dataloader))
    ...
    plt.savefig('./imRED/Cimages_RED.png')
    plt.show()
...
if attack == 'red':
    ...
    imageio.mimsave('./imRED/Cclosest1MRED.gif', imgs)
else:
    to_pil_image = transforms.ToPILImage()
    imgs = [np.array(to_pil_image(img)) for img in closeAL]
    imageio.mimsave('./imTR/Cclosest1MTRAIL.gif', imgs)